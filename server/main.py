...
def _init_logging():
    logger = logging.getLogger("eb_logs")
...
def main(host="localhost", port=8086, username="", password=""):
    ...
    # execute_import(client, db_name. logger)


def execute_import(client, db_name, logger):
    ...
    for i in range(0, 10_000_000):
        ...
        if i % 5000 is 0 and i is not 0:
            logger.info("Created another 500 records")
            logger.info("Measurements: " + str(list(map(lambda m: m["name"], client.get_list_measurements()))))


if __name__ == "__main__":
    main(host="localhost", port=8086, username="", password="")
